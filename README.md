...
- [GUI walkthrough (real screenshots)](#gui-walkthrough-real-screenshots)
...
## GUI walkthrough (real screenshots)

These are genuine screenshots of `app.py` running locally (`streamlit run
app.py`), captured with a headless browser driving the live app — not
mockups.

**1. Initial state — upload prompt**

![GUI initial state](docs/screenshots/gui_initial.png)

**2. Sample input** — a synthetic bracket photo (4 bolt-like dark circles
near the corners, generated for this demo since real photos require sourced
data — see [Current status & limitations](#current-status--limitations-read-this)):

![Sample input image](docs/screenshots/sample_input.jpg)

**3. After inspection — annotated image + verdict panel**

![GUI result](docs/screenshots/gui_result.png)

This run genuinely demonstrates the system's honesty mechanism in action:
it's using the stock fallback detector (no fine-tuned model exists yet — see
the sidebar warning), so two of the four bolt-like shapes were picked up as
low-confidence, unrelated COCO detections and matched within tolerance,
while the top two register as missing. The result is a correct `UNSAFE`
verdict with the exact reasons listed — this is the low-confidence guard and
the "don't trust this verdict yet" banner from
[How the safety verdict is decided](#how-the-safety-verdict-is-decided) and
[Current status & limitations](#current-status--limitations-read-this)
working exactly as designed, not a cherry-picked success case.
...
**Documentation tooling (optional):** `docs/PROJECT_DOCUMENTATION.docx` is a
Word-format mirror of this README, generated by `docs/generate_docx.py`. It
is not needed to run the system — only install `requirements-dev.txt` if you
want to regenerate it after editing this README:
